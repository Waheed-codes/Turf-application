...
export function AnalyticsChart({ trend }: { trend: ManagerAnalytics["trend"] }) {
  ...
  return <div className="rounded-2xl border border-neutral-200 bg-white p-3 shadow-sm">
    <svg role="img" aria-labelledby="revenue-chart-title revenue-chart-description" viewBox="0 0 332 212" className="block h-auto w-full">
      ...
      {[0, 0.5, 1].map((fraction) => <g key={fraction}><line x1="48" x2="312" y1={176 - fraction * 138} y2={176 - fraction * 138} stroke="#e5e5e5" /><text x="43" y={180 - fraction * 138} textAnchor="end" fontSize="9" fill="#737373">{fraction === 0 ? "₹0" : new Intl.NumberFormat("en-IN", { notation: "compact", maximumFractionDigits: 1 }).format(maximum * fraction)}</text></g>)}
      <path d={`${line} L312,176 L48,176 Z`} fill="#f5f5f5" fillOpacity="0.7" />
      <path d={line} fill="none" stroke="#171717" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />
      {points.map((point) => <g key={point.from}><circle cx={point.x} cy={point.y} r="3" fill={point.revenue === null ? "white" : "#171717"} stroke="#171717"><title>{point.from} to {point.to}: {point.revenue === null ? "No known revenue" : bookingAmount(point.revenue)}</title></circle><text x={point.x} y="193" textAnchor="middle" fontSize="10" fill="#737373">{point.label}</text></g>)}
    </svg>
