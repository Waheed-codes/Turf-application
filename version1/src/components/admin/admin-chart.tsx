...
export function AdminTrendChart({ points }: { points: TrendPoint[] }) {
  ...
  return <div><svg role="img" aria-label={`Daily bookings over ${points.length} days. First day: ${points[0]?.count ?? 0}; last day: ${points.at(-1)?.count ?? 0}. Detailed counts below.`} viewBox={`0 0 ${width} ${height}`} className="mt-5 h-auto w-full overflow-visible">
    {[0, max / 3, max * 2 / 3, max].map((value) => <g key={value}><line x1={left} x2={width - right} y1={y(value)} y2={y(value)} stroke="#eeeeee" /><text x={left - 7} y={y(value) + 4} textAnchor="end" fill="#737373" fontSize="11">{Math.round(value)}</text></g>)}
    <path d={`${line} L${x(points.length - 1)},${baseline} L${left},${baseline} Z`} fill="#f5f5f5" />
    <path d={line} fill="none" stroke="#262626" strokeWidth="2" strokeLinejoin="round" />
    {points.map((point, index) => <circle key={point.day} cx={x(index)} cy={y(point.count)} r={points.length > 30 ? 1.8 : 2.7} fill="#262626"><title>Day {point.day}: {point.count} bookings</title></circle>)}
    {ticks.map((index) => <text key={index} x={x(index)} y={height - 9} textAnchor="middle" fill="#737373" fontSize="11">D{points[index].day}</text>)}
...
export function AdminSportChart({ sports }: { sports: SportSlice[] }) {
  ...
  return <div className="mt-5 flex flex-wrap items-center justify-center gap-x-5 gap-y-4">
    <svg role="img" aria-label={`Bookings by sport: ${sports.map((sport) => `${sport.name} ${sport.percentage}%`).join(", ")}.`} viewBox="0 0 240 240" className="w-full min-w-0 max-w-[265px] flex-1 basis-[210px]">
      ...
      })}<text x="120" y="116" textAnchor="middle" fill="#171717" fontSize="25" fontWeight="600">100%</text><text x="120" y="137" textAnchor="middle" fill="#737373" fontSize="11">All bookings</text>
    </svg>
