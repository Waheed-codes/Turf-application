...
export default function VenueDetails({ venue }: { venue: Venue }) {
  ...
  return (
    <main className="min-h-svh bg-neutral-50 pb-[calc(8rem+env(safe-area-inset-bottom))] font-sans text-neutral-950">
      <div className="mx-auto w-full max-w-md">
        <div className="relative h-64 bg-neutral-200 sm:h-80">
          ...
          <div className="absolute right-6 bottom-10 flex gap-3">
            <button type="button" aria-label={favorite ? "Remove from favorites" : "Add to favorites"} aria-pressed={favorite} onClick={() => { setFavorite(!favorite); setMessage(favorite ? "Removed from preview favorites." : "Added to preview favorites for this visit."); }} className={action}><HomeIcon name="heart" className={`size-5 ${favorite ? "fill-neutral-950" : ""}`} /></button>
        ...
        <article className="relative mx-4 -mt-6 rounded-3xl border border-neutral-100 bg-white px-5 py-6 sm:px-6">
          ...
          <p className="mt-3 text-xs leading-5 text-neutral-500">Sample venue information and pricing. No date or time selected.</p>
      ...
      <div className="fixed inset-x-6 bottom-[calc(1rem+env(safe-area-inset-bottom))] mx-auto max-w-sm rounded-full border border-neutral-100 bg-white p-2 shadow-[0_8px_30px_rgba(0,0,0,0.12)]">
        <Link href={`/venues/${venue.id}/book`} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-full bg-black px-6 py-2 font-semibold text-white hover:bg-neutral-800 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-black">Book Now <span aria-hidden="true">→</span></Link>
      </div>
    </main>
  );
}
