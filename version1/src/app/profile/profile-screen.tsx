...
const placeholders: Record<string, string> = {
  "Personal Details": "Editing personal information will be available soon.",
  "Payment Methods": "Managing payment methods will be available soon.",
  "Notification Settings": "Notification preferences will be available soon.",
  ...
  return <main className="min-h-svh bg-neutral-50 px-6 pt-8 pb-[calc(8rem+env(safe-area-inset-bottom))] font-sans text-neutral-950">
    <div className="mx-auto w-full max-w-sm">
      ...
      <section aria-label="Your profile" className="mt-6 flex items-center gap-3 rounded-2xl border border-neutral-200 bg-white p-4 shadow-xs">
        ...
        <div className="min-w-0 flex-1"><h2 className="text-base font-semibold [overflow-wrap:anywhere]">{mockUser.name}</h2><p className="mt-1 text-xs text-neutral-500">{mockUser.phone}</p></div>
        <button type="button" aria-label="Edit personal details" onClick={() => open("Personal Details")} className={`flex size-11 shrink-0 items-center justify-center rounded-full text-neutral-600 hover:bg-neutral-100 active:bg-neutral-200 ${focus}`}><Icon name="edit" /></button>
      </section>
      <dl className="mt-5 grid grid-cols-3 divide-x divide-neutral-200 py-3 text-center">
        {[["Bookings", mockUser.bookings], ["Fav sport", mockUser.favoriteSport], ["Member since", mockUser.memberSince]].map(([label, value]) => <div key={label} className="min-w-0 px-1"><dt className="text-[0.5625rem] font-medium tracking-wide text-neutral-500 uppercase">{label}</dt><dd className="mt-1 text-xs font-semibold sm:text-sm">{value}</dd></div>)}
      </dl>
      <section aria-labelledby="account" className="mt-6"><h2 id="account" className="text-xs font-semibold tracking-widest text-neutral-500">ACCOUNT</h2><ul className="mt-3 overflow-hidden rounded-2xl border border-neutral-200 bg-white divide-y divide-neutral-100">
        {row("Personal Details", <HomeIcon name="profile" />)}
        {row("Payment Methods", <Icon name="payment" />)}
        {row("Favourites", <HomeIcon name="heart" />, "/favorites")}
      </ul></section>
      <section aria-labelledby="preferences" className="mt-6"><h2 id="preferences" className="text-xs font-semibold tracking-widest text-neutral-500">PREFERENCES</h2><ul className="mt-3 overflow-hidden rounded-2xl border border-neutral-200 bg-white">{row("Notification Settings", <Icon name="notifications" />)}</ul></section>
