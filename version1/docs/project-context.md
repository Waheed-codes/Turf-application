...
OTP screen: `/otp` follows the supplied monochrome reference with the existing LOGO and CTA treatment, a back link to Login, recipient display, six digit boxes, Resend, and Verify. The preview explicitly states that no code was sent. Inputs accept ASCII digits, advance focus, move back from an empty box, support arrow navigation and six-digit paste. Verify is disabled until complete. Verify and Resend only show deferred-feature notices; no authentication occurs. Direct visits/reloads without a recipient show a link to Login.
